...
@section('content')
    <div class="d-flex align-items-center justify-content-between">
        <div class="h3 mb-0">@lang('transAdmin.vacancy')</div>
        <a href="{{ route('admin.vacancy.create') }}" class="btn btn-sm btn-danger">
            <i class="fas fa-plus"></i> @lang('transAdmin.vacancy')
        </a>
    </div>
    ...
    <table class="table shadow table-bordered table-striped table-hover table-md bg-white mt-4">
        ...
        @forelse($objs as $obj)
            <tr>
                ...
                <td>@if($obj->img)
                        <img src="{!! Request::root() !!}{!! Storage::disk('local')->url("vacancy/image/".$obj->img) !!}" width="150px">
                    @else
                ...
                <td>
                    <a href="{{ route('admin.vacancy.edit', $obj->id) }}" class="btn btn-outline-success btn-sm mb-1">
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body text-right">
                                    <form action="{{ route('admin.vacancy.delete', $obj->id) }}" method="post">
                                        {{ csrf_field() }}
                                        {{ method_field('DELETE') }}
        ...
        @empty
            <tr>
                <td colspan="5" class="text-center bg-light text-secondary">
                    <i class="fas fa-exclamation-circle"></i> @lang('transAdmin.vacancy') @lang('transAdmin.not-found')
                </td>
            </tr>
        @endforelse
        </tbody>
