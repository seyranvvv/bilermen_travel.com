...
@section('content')
    <div class="d-flex align-items-center justify-content-between">
        ...
        <a href="{{ route('admin.aboutBanner.create') }}" class="btn btn-sm btn-danger">
    ...
    <table class="table shadow table-bordered table-striped table-hover table-md bg-white mt-4">
        ...
        @forelse($objs as $obj)
            <tr>
                <td>{!! $obj->id !!}</td>
                <td>@if($obj->img)
                        <img src="{!! Request::root() !!}{!! Storage::disk('local')->url("aboutBanner/image/".$obj->img) !!}" width="150px">
                    @else
                        <img src="{{ asset('front/placeholder') }}" alt="@lang('transAdmin.not-found')"
                ...
                <td>@lang("transFront.$obj->type")</td>
                <td>
                    <a href="{{ route('admin.aboutBanner.edit', $obj->id) }}" class="btn btn-outline-success btn-sm mb-1">
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body text-right">
                                    <form action="{{ route('admin.aboutBanner.delete', $obj->id) }}" method="post">
                                        {{ csrf_field() }}
                                        {{ method_field('DELETE') }}
