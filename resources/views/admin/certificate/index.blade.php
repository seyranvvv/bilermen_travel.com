...
@section('content')
    <div class="d-flex align-items-center justify-content-between">
        ...
        <a href="{{ route('admin.certificate.create') }}" class="btn btn-sm btn-danger">
            <i class="fas fa-plus"></i> @lang('transAdmin.customer')
        </a>
    </div>
        ...
        <tbody>
            @forelse($objs as $obj)
                <tr>
                    ...
                    <td>
                        @if ($obj->img)
                            <img src="{!! Request::root() !!}{!! Storage::disk('local')->url('certificate/image/' . $obj->img) !!}" width="150px">
                        @else
                            <img src="{{ asset('admin/vontactaphoto/sssss.webp') }}" alt="@lang('transAdmin.not-found')"
                    ...
                    <td>
                        <a href="{{ route('admin.certificate.edit', $obj->id) }}"
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body text-right">
                                        <form action="{{ route('admin.certificate.delete', $obj->id) }}" method="post">
                                            {{ csrf_field() }}
                                            {{ method_field('DELETE') }}
            ...
            @empty
                <tr>
                    <td colspan="5" class="text-center bg-light text-secondary">
                        <i class="fas fa-exclamation-circle"></i>@lang('transAdmin.customer') @lang('transAdmin.not-found')
                    </td>
                </tr>
            @endforelse
        </tbody>
